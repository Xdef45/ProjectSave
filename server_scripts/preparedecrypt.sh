...
REVERSE_PORT="${2:?Usage: $0 CLIENT REVERSE_PORT}" # port génér pour reverse
LOCAL_USER="${3:?Usage: $0 CLIENT REVERSE_PORT}"

SERVER_TO_CLIENT_KEY="/home/tunnel/.ssh/server_to_client_ed25519" #clé à feed pour pouvoir reverse
SERVER_GPG_PASSFILE="/etc/backup_secrets/key.pass" # clé gpg
...
# 1) pull fichier chiffré client -> stockage temporaire
...
# 2) déchiffre vers dosssier temp
...
# 4) Nettoyage des clés côté server
rm -f "$PLAIN_LOCAL"
rm -f "$ENC_LOCAL"
