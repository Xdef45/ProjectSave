...
pub async fn get_master_key_2_encrypted(uuid:&String, ssh_connexion: Arc<Session>, sftp_connexion: Arc<Sftp>)->Result<String, APIError>{
...
pub async fn get_master_key_1_encrypted(uuid:&String, ssh_connexion: Arc<Session>, sftp_connexion: Arc<Sftp>)-> Result<String, APIError>{
        // Ouverture de la clé borg 1
    let path_key = format!("{}/{}/bootsrap/{}.gpg", CLIENT_DIRECTORY,uuid, uuid);
    println!("{}", path_key);
    let mut master_key_file = match sftp_connexion.open(&path_key).await {
        Ok(f)=>f,
        Err(_)=> {
